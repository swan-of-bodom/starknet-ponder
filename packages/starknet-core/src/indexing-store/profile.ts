  ...
  switch (event.type) {
    case "block": {
      ...
      if (
        ...
        eq(event.event.block.sequencerAddress, value)
      ) {
        return { type: "derived", value: ["block", "sequencerAddress"] };
      }
    ...
    case "transaction": {
      ...
      if (
        ...
        eq(event.event.transaction.senderAddress, value)
      ) {
        return { type: "derived", value: ["transaction", "senderAddress"] };
      }

      // Starknet doesn't have 'to' field
    ...
    case "log": {
      ...
      // Starknet uses sequencerAddress instead of miner
      if (event.event.block.sequencerAddress && eq(event.event.block.sequencerAddress, value)) {
        return { type: "derived", value: ["block", "sequencerAddress"] };
      }
      ...
      // Starknet uses senderAddress instead of from
      if (event.event.transaction.senderAddress && eq(event.event.transaction.senderAddress, value)) {
        return { type: "derived", value: ["transaction", "senderAddress"] };
      }
