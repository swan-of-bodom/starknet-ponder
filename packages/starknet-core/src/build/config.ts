...
export async function buildIndexingFunctions({
    ...
    if (blockNumberOrTag === "latest") {
      if (perChainLatestBlockNumber.has(chain.name)) {
        ...
        const blockPromise = rpc
          .request(
            {
              method: "starknet_blockNumber" as any,
              params: [] as any,
            },
            context,
          )
          .then((block) => {
            // starknet_blockNumber returns just the number
            if (block === null || block === undefined)
              throw new BlockNotFoundError({ blockNumber: "latest" as any });
            return typeof block === "number"
              ? block
              : hexToNumber(block as any);
          })
          .catch((e) => {
            throw new Error(
  ...
  const finalizedBlocks = await Promise.all(
    chains.map((chain) => {
      ...
      const blockPromise = _starknet_getBlockByNumber(rpc, {
        blockTag: "latest",
      })
        .then((block) => {
          const blockNum = block.number ?? (block as any).block_number;
          return typeof blockNum === "number"
            ? blockNum
            : hexToNumber(blockNum);
        })
        .catch((e) => {
          throw new Error(
            `Unable to fetch "latest" block for chain '${chain.name}':\n${e.message}`,
      ...
      return blockPromise.then((latest) =>
        _starknet_getBlockByNumber(rpc, {
    ...
    await Promise.all(
      flattenSources(config.contracts ?? {}).map(
        async (source): Promise<ContractSource[]> => {
          ...
          const abiEvents = buildAbiEvents({ abi: source.abi as any });
          const abiFunctions = buildAbiFunctions({ abi: source.abi as any });
          ...
          for (const logEvent of registeredLogEvents) {
            ...
            registeredEventSelectors.push(abiEvent.selector as Hex);
          }
          ...
          for (const _function of registeredCallTraceEvents) {
            ...
            registeredFunctionSelectors.push(abiFunction.selector as Hex);
          }
          ...
          if (source.filter !== undefined) {
            ...
            for (const selector of filteredEventSelectors) {
              if (registeredEventSelectors.includes(selector) === false) {
                throw new Error(
                  `Validation failed: Event selector '${(abiEvents.bySelector as any)[selector]?.safeName}' is used in a filter but does not have a corresponding indexing function.`,
                );
              }
            }

            // NOTE: Topis is useless???
          ...
          let resolvedAddress = source?.address;

          // Normalize stark addresses to ensure they're padded
          if (typeof resolvedAddress === "string") {
            resolvedAddress = normalizeAddress(resolvedAddress);
          } else if (Array.isArray(resolvedAddress)) {
            resolvedAddress = resolvedAddress.map((addr) =>
              normalizeAddress(addr),
            );
          } else if (
            typeof resolvedAddress === "object" &&
            resolvedAddress !== null &&
            "address" in resolvedAddress
          ) {
            const factoryAddress = resolvedAddress.address;
            if (typeof factoryAddress === "string") {
              resolvedAddress = {
                ...resolvedAddress,
                address: normalizeAddress(factoryAddress),
              };
            } else if (Array.isArray(factoryAddress)) {
              resolvedAddress = {
                ...resolvedAddress,
                address: factoryAddress.map((addr) => normalizeAddress(addr)),
              };
            }
          }
          ...
          if (
            ...
            for (const address of Array.isArray(resolvedAddress)
              ...
              // Validate address length: 66 chars for Starknet
              if (address!.length !== 66)
                throw new Error(
                  `Validation failed: Invalid length for address '${address}'. Got ${address!.length}, expected 66 characters.`,
                );
            }
          }
  ...
  const chains: Chain[] = Object.entries(config.chains).map(
    ([chainName, chain]) => {
        ...
        typeof chain.id === "string"
          ? (() => {
              const STARKNET_CHAIN_ID_MAP: Record<string, number> = {
                ...
                "0x534e5f5345504f4c4941": 2, // SN_SEPOLIA
              };
              return STARKNET_CHAIN_ID_MAP[chain.id] || 0;
            })()
          : chain.id;
      ...
      if (typeof chain.rpc === "string" || Array.isArray(chain.rpc)) {
        ...
        if (rpcs.length === 0) {
          throw new Error(
            `Chain "${chainName}" with id ${chainId} has no RPC URLs.`,
          );
        }
...
export async function safeBuildIndexingFunctions({
  ...
  try {
    ...
    buildError.stack = (_error as Error).stack; // Preserve stack trace for debugging
