        ...
        configBuild.rpcs.map(async (rpc, index) => {
          ...
          try {
            const chainId = await rpc.request(
              { method: "starknet_chainId", params: [] } as any,
              context,
            );

            const numericChainId = STARKNET_CHAIN_ID_MAP[chainId as string];

            if (numericChainId === undefined) {
              common.logger.warn({
                msg: "Unknown Starknet chain ID from JSON-RPC",
                chain: chain.name,
                configured_chain_id: chain.id,
                rpc_chain_id: chainId,
              });
            } else if (numericChainId !== chain.id) {
              common.logger.warn({
                msg: "Configured chain ID does not match JSON-RPC response",
                chain: chain.name,
                configured_chain_id: chain.id,
                rpc_chain_id: numericChainId,
                rpc_chain_id_hex: chainId,
              });
            }
          } catch (e) {
            const error = new RetryableError("Failed to connect to Starknet JSON-RPC");
            error.stack = undefined;
            return { status: "error", error } as const;
          }
