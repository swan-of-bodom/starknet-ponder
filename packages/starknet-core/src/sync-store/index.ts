...
import { getBlockIdParam } from "@/indexing/client.js";
...
export const createSyncStore = ({
  ...
  const syncStore = {
                ...
                transactions
                  ...
                  .map((transaction) =>
                    encodeTransaction({
                      transaction,
                      chainId,
                      blockNumber: (transaction as any).blockNumber,
                    }),
                  ),
              )
              .onConflictDoNothing({
    ...
    getEventData: async (
      ...
      // Note: DB schema uses EVM column names (from, to, status) but internal types use Starknet names (senderAddress, executionStatus)
      // We use looser types here and do the mapping in the decode logic below
      type TraceSelect = {
      ...
      // Starknet-native transaction select
      const transactionSelect: Record<string, unknown> = {
        ...
        senderAddress: PONDER_SYNC.transactions.senderAddress,
        // Type-specific fields - always select them so they're available based on tx type
        contractAddress: PONDER_SYNC.transactions.contractAddress,
        entryPointSelector: PONDER_SYNC.transactions.entryPointSelector,
        classHash: PONDER_SYNC.transactions.classHash,
        compiledClassHash: PONDER_SYNC.transactions.compiledClassHash,
        contractAddressSalt: PONDER_SYNC.transactions.contractAddressSalt,
        constructorCalldata: PONDER_SYNC.transactions.constructorCalldata,
      };
      ...
      // Starknet-native transaction receipt select
      const transactionReceiptSelect: Record<string, unknown> = {
        ...
        executionStatus: PONDER_SYNC.transactionReceipts.executionStatus,
        // Always select Starknet-specific fields
        actualFee: PONDER_SYNC.transactionReceipts.actualFee,
        executionResources: PONDER_SYNC.transactionReceipts.executionResources,
        finalityStatus: PONDER_SYNC.transactionReceipts.finalityStatus,
        messagesSent: PONDER_SYNC.transactionReceipts.messagesSent,
        events: PONDER_SYNC.transactionReceipts.events,
      };

      for (const column of unionFilterIncludeTransactionReceipt(filters)) {
        // Skip fields already included above
        if (column === 'actualFee' || column === 'events' || column === 'executionResources' ||
            column === 'executionStatus' || column === 'finalityStatus' || column === 'messagesSent' ||
            column === 'transactionHash') {
          continue;
        }
        // @ts-ignore
        transactionReceiptSelect[column] =
          PONDER_SYNC.transactionReceipts[column as keyof typeof PONDER_SYNC.transactionReceipts];
      }
      ...
      const logsQuery = database.syncQB.raw
        .select({
          ...
          blockHash: PONDER_SYNC.logs.blockHash,
          transactionHash: PONDER_SYNC.logs.transactionHash,
          topic0: PONDER_SYNC.logs.topic0,
          topic1: PONDER_SYNC.logs.topic1,
          topic2: PONDER_SYNC.logs.topic2,
          topic3: PONDER_SYNC.logs.topic3,
          allKeys: PONDER_SYNC.logs.allKeys,  // For Starknet: retrieve all keys
      ...
      for (let i = 0; i < blocksRows.length; i++) {
        ...
        const block = blocksRows[i]! as any;
        const internalBlock = block as unknown as InternalBlock;

        // Parse JSON fields from DB
        internalBlock.l1GasPrice = typeof block.l1GasPrice === "string" ? JSON.parse(block.l1GasPrice) : block.l1GasPrice;
        internalBlock.l1DataGasPrice = typeof block.l1DataGasPrice === "string" ? JSON.parse(block.l1DataGasPrice) : block.l1DataGasPrice;
      }
      ...
      for (let i = 0; i < transactionsRows.length; i++) {
        ...
        const transaction = transactionsRows[i]! as any;
        ...
        // Parse JSON fields from DB
        internalTransaction.calldata = transaction.calldata ? JSON.parse(transaction.calldata) : null;
        internalTransaction.signature = transaction.signature ? JSON.parse(transaction.signature) : null;
        internalTransaction.resourceBounds = transaction.resourceBounds ? JSON.parse(transaction.resourceBounds) : null;
        internalTransaction.paymasterData = transaction.paymasterData ? JSON.parse(transaction.paymasterData) : null;
        internalTransaction.accountDeploymentData = transaction.accountDeploymentData ? JSON.parse(transaction.accountDeploymentData) : null;
        internalTransaction.constructorCalldata = transaction.constructorCalldata ? JSON.parse(transaction.constructorCalldata) : null;

        // nonce stays as Hex (not converted to bigint)
        internalTransaction.nonce = transaction.nonce ?? null;

        // Type-specific fields (already in correct format, just ensure they're present)
        internalTransaction.contractAddress = transaction.contractAddress ?? null;
        internalTransaction.entryPointSelector = transaction.entryPointSelector ?? null;
        internalTransaction.classHash = transaction.classHash ?? null;
        internalTransaction.compiledClassHash = transaction.compiledClassHash ?? null;
        internalTransaction.contractAddressSalt = transaction.contractAddressSalt ?? null;
      }
      ...
      for (let i = 0; i < transactionReceiptsRows.length; i++) {
        ...
        const transactionReceipt = transactionReceiptsRows[i]! as any;
        ...
        // Parse JSON fields from DB (new Starknet-native schema)
        const parsedActualFee = typeof transactionReceipt.actualFee === "string"
          ? JSON.parse(transactionReceipt.actualFee)
          : transactionReceipt.actualFee;
        // actualFee is stored in camelCase
        internalTransactionReceipt.actualFee = parsedActualFee;

        const parsedResources = typeof transactionReceipt.executionResources === "string"
          ? JSON.parse(transactionReceipt.executionResources)
          : transactionReceipt.executionResources;
        internalTransactionReceipt.executionResources = {
          l1DataGas: parsedResources.l1DataGas ?? parsedResources.dataAvailability?.l1DataGas ?? 0,
          l1Gas: parsedResources.l1Gas ?? parsedResources.dataAvailability?.l1Gas ?? 0,
          l2Gas: parsedResources.l2Gas ?? 0,
        };

        internalTransactionReceipt.messagesSent = typeof transactionReceipt.messagesSent === "string"
          ? JSON.parse(transactionReceipt.messagesSent)
          : transactionReceipt.messagesSent;

        const parsedEvents = typeof transactionReceipt.events === "string"
          ? JSON.parse(transactionReceipt.events)
          : transactionReceipt.events;
        internalTransactionReceipt.events = parsedEvents.map((e: any) => ({
          data: e.data,
      ...
      for (let i = 0; i < logsRows.length; i++) {
        ...
        if (log.allKeys) {
          internalLog.keys = JSON.parse(log.allKeys);
          // @ts-ignore
    ...
    getRpcRequestResults: async ({ requests, chainId }, context) => {
      ...
      for (let i = 0; i < requests.length; i++) {
        ...
        const blockNumber = getBlockIdParam(request);
    ...
    pruneRpcRequestResults: async ({ blocks, chainId }, context) => {
      ...
      const numbers = blocks.map(({ number }) => BigInt(number));
...
export const transactionFilter = (filter: TransactionFilter): SQL => {
  ...
  // Starknet uses senderAddress instead of from, and doesn't have 'to'
  conditions.push(
    addressFilter(filter.fromAddress, PONDER_SYNC.transactions.senderAddress),
  );
  // Note: Starknet transactions don't have a 'to' field - toAddress filter is ignored
