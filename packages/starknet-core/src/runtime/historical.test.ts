...
test("getLocalEventGenerator()", async (context) => {
  ...
  const syncProgress = await getLocalSyncProgress({
    ...
    chain,
    rpc,
    finalizedBlock: await _starknet_getBlockByNumber(rpc, { blockNumber: 1 }),
    cachedIntervals,
  });
...
test("getLocalEventGenerator() pagination", async (context) => {
  ...
  const syncProgress = await getLocalSyncProgress({
    ...
    chain,
    rpc,
    finalizedBlock: await _starknet_getBlockByNumber(rpc, { blockNumber: 2 }),
    cachedIntervals,
  });
  ...
  const events = await drainAsyncGenerator(
    getHistoricalEventsMultichain({
      ...
      indexingBuild: {
        ...
        chains: [chain],
        rpcs: [rpc],
        finalizedBlocks: [await _starknet_getBlockByNumber(rpc, { blockNumber: 1 })],
      },
  ...
  const events = await drainAsyncGenerator(
    getHistoricalEventsMultichain({
      ...
      indexingBuild: {
        ...
        chains: [chain],
        rpcs: [rpc],
        finalizedBlocks: [await _starknet_getBlockByNumber(rpc, { blockNumber: 2 })],
      },
