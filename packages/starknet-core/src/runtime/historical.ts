  ...
  while (true) {
    ...
    const finalizedBlocks = await Promise.all(
      params.indexingBuild.chains.map((chain, i) => {
        ...
        return _starknet_getBlockByNumber(rpc, { blockTag: "latest" }, context)
          .then((latest) =>
            _starknet_getBlockByNumber(
              rpc,
              {
                blockNumber: Math.max(
                  latest.number - chain.finalityBlockCount,
                  0,
                ),
              },
              context,
...
export async function* getLocalSyncGenerator(params: {
  ...
  let cursor = params.syncProgress.start.number;
  const last =
    params.syncProgress.end === undefined
      ? params.syncProgress.finalized
      : params.syncProgress.end.number >
  ...
  if (
    params.syncProgress.start.number >
    params.syncProgress.finalized.number
  ) {
    ...
    params.common.logger.info({
      ...
      chain: params.chain.name,
      chain_id: params.chain.id,
      finalized_block: params.syncProgress.finalized.number,
      start_block: params.syncProgress.start.number,
    });

    params.common.metrics.ponder_sync_block.set(
      label,
      params.syncProgress.current.number,
    );
    params.common.metrics.ponder_sync_block_timestamp.set(
      label,
      params.syncProgress.current.timestamp,
    );
  ...
  if (params.syncProgress.current !== undefined) {
    ...
    yield params.syncProgress.current.number;
    ...
    if (
      ...
      if (params.isCatchup === false) {
        params.common.logger.info({
          ...
          chain: params.chain.name,
          chain_id: params.chain.id,
          cached_block: params.syncProgress.current.number,
          cache_rate: "100%",
        });
      }
      return;
    ...
    cursor = params.syncProgress.current.number + 1;
  } else {
    params.common.logger.info({
      msg: "Started fetching backfill JSON-RPC data",
      chain: params.chain.name,
  ...
  while (true) {
    ...
    const interval: Interval = [
      Math.min(cursor, last.number),
      Math.min(cursor + estimateRange, last.number),
    ];
