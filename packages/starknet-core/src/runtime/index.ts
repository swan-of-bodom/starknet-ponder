...
export async function getLocalSyncProgress(params: {
  ...
  const syncProgress = {
    isEnd: () => {
      ...
      return syncProgress.current.number >= syncProgress.end.number;
    },
    isFinalized: () => {
      ...
      // block.number is now a plain number, not hex
      return syncProgress.current.number >= syncProgress.finalized.number;
    },
    getCheckpoint: ({ tag }) => {
  ...
  // params.finalizedBlock.number is now a plain number, not hex
  if (end > params.finalizedBlock.number) {
    syncProgress.end = {
      number: end,
      hash: "0x",
      parentHash: "0x",
      // Convert bigint to number for LightBlock
      timestamp: Number(MAX_CHECKPOINT.blockTimestamp),
    } satisfies LightBlock;
  } else {
    syncProgress.end = await _starknet_getBlockByNumber(
      params.rpc as any,
      { blockNumber: end },
    );
  }
