// NOTE: Same as /core, except:
//       - Checkpoint varchar length 79 instead of 75
//       - Starknet chain IDs are larger 
//       - block data is numeric, not hex
...
export const getPonderCheckpointTable = (schema?: string) => {
  if (schema === undefined || schema === "public") {
    return pgTable("_ponder_checkpoint", (t) => ({
      ...
      safeCheckpoint: t.varchar({ length: 79 }).notNull(),
      latestCheckpoint: t.varchar({ length: 79 }).notNull(),
      finalizedCheckpoint: t.varchar({ length: 79 }).notNull(),
    }));
  }
...
CREATE TABLE IF NOT EXISTS "${namespace.schema}"."_ponder_checkpoint" (
  ...
  "safe_checkpoint" VARCHAR(79) NOT NULL,
  "latest_checkpoint" VARCHAR(79) NOT NULL,
  "finalized_checkpoint" VARCHAR(79) NOT NULL
)`,
            ),
          context,
        );
      ...
      const tryAcquireLockAndMigrate = () =>
        adminQB.transaction({ label: "migrate" }, async (tx) => {
          ...
          for (const { chainId, finalizedCheckpoint } of checkpoints) {
            ...
            // finalizedBlock.timestamp is now a plain number, not hex
            if (
