...
type AbiConfig<abi extends StarknetAbi | readonly unknown[]> = {
...
type ContractConfig<chains, abi extends StarknetAbi | readonly unknown[]> = Prettify<
  AbiConfig<abi> &
...
type GetContract<chains = unknown, contract = unknown> = contract extends {
  abi: infer abi extends StarknetAbi | readonly unknown[];
}
  ? // 1. Contract has a valid abi
    ContractConfig<chains, abi>
  : // 2. Contract has an invalid abi
    ContractConfig<chains, StarknetAbi>;
