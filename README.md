...
For EVM-based chains (Ethereum, Polygon, Arbitrum, etc.), use the original `ponder` npm package. The original functionality is fully preserved in this fork (from Ponder version `0.14.3`) if one day we implement EVM + Starknet indexing in the same package.
